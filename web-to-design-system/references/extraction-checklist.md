...
一个 URL 通常不够。按目标系统的类型挑 2～4 个页面一起取证（`extract-evidence.mjs <url1> <url2> …`）：

| 想拿到 | 去哪个页面 |
| --- | --- |
| 正文 / 标题 / 链接 / 主按钮 / 卡片 | 首页、落地页 |
| 输入框、占位符、焦点样式、表单标签 | 登录 / 注册 / 搜索 / 设置页 |
| 表格、状态胶囊、分页、选中行 | 列表页、后台工作台 |
...
`draft-tokens.mjs` 写出的 `audit-summary.md` 开头四行：来源、首屏模式与另一模式、品牌族判定、角色覆盖（观察 / 推断 / 缺口）。品牌族判错（比如把状态红当品牌）→ 加 `--brand #hex` 重跑；缺口里有 core 项 → 回到「0. 选页面」补证据，或按 [mapping-rules.md](mapping-rules.md)「缺口怎么补」处理。
