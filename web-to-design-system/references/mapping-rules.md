...
先看系统类型（`--profile`，`draft-notes.json` 的 `profile`）：它决定哪些角色进「必须处理的缺口」、哪些只是「可选角色未填」。

| 系统类型 | 必须处理 | 允许按规则推断 | 不推断、不借值 |
| --- | --- | --- | --- |
| brand 品牌 / 内容站 | core 去掉产品专属的 24 个（状态色 ×10、危险色 ×5、输入框三件、浮层表面、控件 sm / lg、dropdown / toast 层、禁用透明度） | 必须集 + 品牌面 / 大字 / 链接装饰 / 浮层阴影 / 缓动 | 状态、危险、选中态、骨架屏、只读、图标库、表格、壳层、图表 |
| product 产品 UI | core 全部（79） | core + extended | shell（侧栏、表格内边距、图表、涨跌、弹窗尺寸） |
...
2. **按规则推断**：用上表「没证据时」一栏的规则手写 alias，`$description` 以 `[推断]` 开头写清依据。状态色可借公司调色板或 Citrine 的色相参考（success 绿 / warning 橙 / error 红 / info 青），但值要按目标系统的中性色调性重算对比度——这一条只对 product / admin 成立。
