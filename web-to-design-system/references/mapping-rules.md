...
先看系统类型（`--type`，`draft-notes.json` 的 `type`；定义见 [system-types.md](system-types.md)）：它决定哪些角色进「必须处理的缺口」、哪些只是「可选角色未填」。内置三类：

| 系统类型 | 必须处理 | 允许按规则推断 | 不推断、不借值 |
| --- | --- | --- | --- |
| `website` 通用网站（别名 brand） | core 去掉产品专属的 24 个（状态色 ×10、危险色 ×5、输入框三件、浮层表面、控件 sm / lg、dropdown / toast 层、禁用透明度） | 必须集 + 品牌面 / 大字 / 链接装饰 / 浮层阴影 / 缓动 | 状态、危险、选中态、骨架屏、只读、图标库、表格、壳层、图表 |
| `product` 产品应用 | core 全部（79） | core + extended | shell（侧栏、表格内边距、图表、涨跌、弹窗尺寸） |
| `admin` 中后台 | core + shell（107） | 全部 | — |

「必须处理的缺口」逐行决定，三选一，都要写进 AUDIT：

1. **补证据**：换一个页面再取证（表单页补输入框 / 状态色，列表页补表格 / 选中行，打开弹窗补遮罩），`extract-evidence.mjs` 支持多 URL。
2. **按规则推断**：用上表「没证据时」一栏的规则手写 alias，`$description` 以 `[推断]` 开头写清依据。状态色可借公司调色板或 Citrine 的色相参考（success 绿 / warning 橙 / error 红 / info 青），但值要按目标系统的中性色调性重算对比度——这一条只对把状态色列为必须处理的类型（内置的 product / admin）成立。
3. **本系统不需要**：在 AUDIT 写一句理由（比如「站点没有固定顶栏，导航是绝对定位的按钮」）。

「可选角色未填」有证据再填，没有就留空。不要为了把词表填满去借 Citrine 的侧栏宽、表格内边距、弹窗尺寸——那是中后台的决定，不是这套系统的；接入方需要时走提案。
