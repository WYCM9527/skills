...
async function main() {
  ...
  if (!tokensOnly) {
    ...
    await put("design-system/AUDIT.md", fill(await readAsset("AUDIT.template.md"), {
      ...
      NAME: name,
      OPTIONAL: String(notes?.counts?.optional ?? notes?.optional?.length ?? 0),
  ...
  if (mode === "into-repo" && !tokensOnly) {
    ...
    const upsert = upsertReadmeRow(await readFile(rootReadmePath, "utf8"), { description: identity.description.replace(/\|/g, "／"), id, name, replaceDescription: options.description !== undefined, version });
    if (upsert.changed) {
