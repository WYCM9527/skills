...
| 侧栏 | `color.bg.sidebar`；项 hover `color.bg.sidebar-hover`；当前 `color.bg.sidebar-selected` | `color.text.sidebar` / `-strong` / `-muted`；当前 `color.text.sidebar-selected` | `color.border.sidebar`；宽 `layout.sidebar.width`，折叠 `layout.sidebar.collapsed-width` |
| 顶栏 | `color.bg.surface` | `color.text.primary` | 高 `layout.topbar.height`，`layer.sticky`，底边线 `color.border.default` |
| 抽屉 | 遮罩 `color.bg.overlay`；面板 `color.bg.elevated` | `color.text.primary` | 宽 `layout.drawer.width`，`elevation.modal.*`，`layer.modal` |
| 横向表单 | — | 标签 `color.text.secondary` | 标签列 `layout.form.label-width`，单列最大宽 `layout.form.max-width` |
| 图表 | 面积 `color.chart.area` | 轴与图例 `color.text.secondary` | 分类 `color.chart.1…6`，顺序 `color.chart.sequential.*`，涨跌 `color.data.*` |
