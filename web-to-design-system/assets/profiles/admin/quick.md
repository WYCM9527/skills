...
9. 壳层尺寸只从 token 拿：侧栏 `layout.sidebar.width` / `-collapsed-width`，顶栏 `layout.topbar.height`，弹窗 `layout.modal.width.*`，抽屉 `layout.drawer.width`，表单标签列 `layout.form.label-width`；表格单元格内边距 `table.cell.padding-*`。
10. 图表只用 `color.chart.*`（分类）与 `color.chart.sequential.*`（顺序），涨跌用 `color.data.increase / decrease`，不在图表里直接写状态色。
11. 改完跑 steward `guard`（构建与登记一致）和 `status`（页面字面量清零）。
