...
| `settle` | 低 | 汇总前两段拒绝自动处理的所有待决项，让人逐组决定：归并（merge）、升级（promote）或豁免（exempt）。`--apply --decisions-file` 落地已确认的归并（把点名的字面量 / 旧变量改写为 `var(--…)`）并写豁免登记册；升级仍走 propose。 |

## 匹配规则（为什么有些值不会被自动替换）

- **Semantic 优先**：治理模型要求消费端用用途层 Token。值只命中 Primitive（调色盘）时不静默降级，进待决清单请人先命名用途（`primitive-only`）。
- **先看属性再看值**：字面量所在的 CSS 属性决定它能匹配哪一类 Token——`font-size: 14px` 只在字号 / 图标尺寸 Token 里找，`padding: 14px` 只在间距 / 单元格内边距里找，`color:` 只找文字 / 状态色，`background:` 只找底色 / 动作色，`border:` 按值的种类分别找边线宽或边线色；Tailwind 任意值按 utility 前缀（`p-[…]`、`text-[…]`、`bg-[…]`、`rounded-[…]`）推出属性。属性能唯一定位到一个 Semantic Token 时直接替换，计划里标 `matchedBy: "property"`；仍有多个候选时待决项只列同类候选，并标 `narrowedBy` 与原候选数；属性未知（`transform` 之类）退回按值匹配。
- **撞值即停**：同一个值对应多个 Semantic Token（如「主按钮蓝」和「链接蓝」同值）且属性也分不开时列为 `ambiguous-semantic`，由人按上下文分派。
- **注释不算证据**：`/* 12px 示意 */`、`// #fff fallback`、`<!-- 24px -->` 里的值不进任何清单（`audit` 同样跳过），字符串与 `url(http://…)` 里的 `//` 不会被当成注释。
...
- 工作区有未提交改动 → 拒绝写入（混在一起无法单独回滚），除非加 `--allow-dirty`（0.5 里这个标志没有生效，0.6 修正）。
...
1. **merge**：这个值其实就是某个已批准 Token → 写进决定文件，`settle --apply` 直接把点名的字面量（可限定属性与文件）或旧变量定义改写为 `var(--…)`，并生成 `MIGRATION.md`；
...
用户在问卷里确认后，Agent 把决定写成一个 JSON 文件再执行（不带 `--apply` 只预览 `mergePlan`）：

```json
{
  "merges": [
    { "value": "14px", "property": "font-size", "token": "text.body.size" },
    { "value": "#111827", "files": ["src/legacy/"], "token": "color.text.primary" },
    { "name": "--brand", "token": "color.action.primary" }
  ],
  "exemptions": [
    { "path": "src/promo.css", "value": "#ff00ff", "reason": "活动专用色，随活动整体下线" }
  ]
...
`value` 按归一化后的值匹配（大小写 / 短 hex / rgb() 同值），`property` 可选、限定只改该属性上的出现，`files` 可选、限定文件或目录前缀，`name` 用于旧变量定义。Token 必须存在，非 Semantic 层会给出警告但允许；JS 里的字面量仍只报告不改写。

```text
node scripts/migrate.mjs --project /absolute/project/path --phase settle --apply --decisions-file /tmp/decisions.json
```

只登记豁免的旧写法 `--exemptions-file` 仍然可用。
...
只读输出：Token／Scope／Theme 计数、可桥接与可替换的剩余数、待决数、豁免数、`var(--…)` 使用率（统一进度百分比），以及**基于当前状态的下一步建议**。跑完不知道下一步干什么时，先看它。它不检查 `dist` 是否陈旧、Token 引用是否合法——那是 `guard` 的事（见 [guard.md](guard.md)「与 status 的分工」）。
