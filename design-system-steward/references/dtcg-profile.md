...
本 Skill 以 [DTCG Format 2025.10](https://www.designtokens.org/tr/2025.10/format/) 的 token、group 和 alias 语义为基础，但只承诺构建一个经过 fixture 验证的 CSS 子集。对已经用户确认纳管的设计决定，DTCG JSON 是具体值与 alias 的唯一真相源；确认迁移前的旧来源仍只是审计证据。
...
Core 表示确认的默认 Theme。构建一个 Theme 时，Core 用于 alias 解析，而 `themes/<id>/tokens/` 只提供相对 Core 的 delta；生成器将其写入 `dist/themes/<id>.css`。Theme 不会自动与 Scope 组合：若二者覆写同一个随 Theme 变化的语义值，Guard 要求人先作决定。
